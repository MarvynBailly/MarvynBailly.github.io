...
void diffuse(std::vector<std::vector<Pheromone>>& grid, float diffusionRate, float evapSpeed, float dt) {
    ...
    for (int x = 0; x < rows; ++x) {
        for (int y = 0; y < cols; ++y) {
            ...
            for (int dx = -1; dx <= 1; ++dx) {
                for (int dy = -1; dy <= 1; ++dy) {
                    ...
                    if (nx >= 0 && nx < rows && ny >= 0 && ny < cols) {
                        antSum += grid[nx][ny].oldAntConcentration;
                        foodSum += grid[nx][ny].oldFoodConcentration;
                        count++;
                    }
                }
            }
            ...
            float antDiffused = lerp(grid[x][y].oldAntConcentration, antBlur, diffusionRate * dt);
            float foodDiffused = lerp(grid[x][y].oldFoodConcentration, foodBlur, diffusionRate * dt);
            float antDiffusedAndEvap = max(0.0f, antDiffused - evapSpeed * dt);
            float foodDiffusedAndEvap = max(0.0f, foodDiffused - evapSpeed * dt);
            grid[x][y].newAntConcentration = antDiffusedAndEvap;
            grid[x][y].newFoodConcentration = foodDiffusedAndEvap;

            // keep feed smelly
            if(grid[x][y].hasFood){
                grid[x][y].newFoodConcentration = 1.0f;
            }

            // keep the base smelly
            if(grid[x][y].hasColony){
                grid[x][y].newAntConcentration = 1.0f;
            }
        }
    }
    #pragma omp parallel for collapse(2)
    ...
    for (int x = 0; x < rows; ++x) {
        for (int y = 0; y < cols; ++y) {
            grid[x][y].oldAntConcentration = grid[x][y].newAntConcentration;
            grid[x][y].oldFoodConcentration = grid[x][y].newFoodConcentration;
        }
    }
}
    ...
    for (int offsetX = -static_cast<int>(sensorSize); offsetX <= static_cast<int>(sensorSize); ++offsetX) {
        for (int offsetY = -static_cast<int>(sensorSize); offsetY <= static_cast<int>(sensorSize); ++offsetY) {
            ...
            if (pos.x >= 0 && pos.x < grid.size() && pos.y >= 0 && pos.y < grid[0].size()) {
                if(pheromoneType == 0){
                    sum += grid[static_cast<int>(pos.x)][static_cast<int>(pos.y)].oldFoodConcentration;
                    //steer hard towards food
                    if(grid[static_cast<int>(pos.x)][static_cast<int>(pos.y)].hasFood){sum+=targetBias;}
                }else{
                    sum += grid[static_cast<int>(pos.x)][static_cast<int>(pos.y)].oldAntConcentration;
...
int main() {
    ...
    const int xNumber = 100;
    const int yNumber = 100;
        ...
        for (int ii = -foodClusterSize; ii < foodClusterSize; ii++){
            for (int jj = -foodClusterSize; jj < foodClusterSize; jj++){
                ...
                if (x >= 0 && x < static_cast<int>(pheromoneGrid.size()) && y >= 0 && y < static_cast<int>(pheromoneGrid[0].size())) {
                    ...
                    pheromoneGrid[x][y].oldFoodConcentration = 1.0f; // add a food pheromone concentration
                }
            }
        }
    }
        ...
        for (int x = 0; x < xNumber; ++x) {
            for (int y = 0; y < yNumber; ++y) {
                ...
                // Determine color based on pheromone concentrations
                    ...
                    float antConcentration = pheromoneGrid[x][y].oldAntConcentration;
                    float foodConcentration = pheromoneGrid[x][y].oldFoodConcentration;
