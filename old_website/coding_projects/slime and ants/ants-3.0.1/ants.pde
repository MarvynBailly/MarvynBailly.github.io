...
void draw(){
  ...
  //rAgent = map(frameCount,0,500,255,100);
  
  //bAgent = map(frameCount,0,500,0,100);
...
void updateDisplay(){
  loadPixels();
  for(int i = 0; i <width*height; i++){
    float value = displayArray[i];
    float r = map(value, 0, 255, rBackground, rAgent);
    float g = map(value, 0, 255, gBackground, gAgent);
    float b = map(value, 0, 255, bBackground, bAgent);
