function Particle(x, y, DNA) {
  ...
  this.DNA = DNA;
  this.team = this.DNA.team;

}

Particle.prototype.sendScore = function(){
  this.DNA.fitness(this.lifeTotal,this.score);
}

Particle.prototype.decode = function(){
  this.chaseMult = this.DNA.chaseMult;
  this.chaseRange = this.DNA.chaseRange;
  this.seperateSize = this.DNA.seperateRange;
  this.seperateMult = this.DNA.seperateMult;
  this.alignRange = this.DNA.alignRange;
  this.alignMult = this.DNA.alignMult;
  this.cohesionRange = this.DNA.cohesionRange;
  this.cohesionMult = this.DNA.cohesionMult;
  this.runRange = this.DNA.runRange;
  this.runMult = this.DNA.runMult;
  this.foodRange = this.DNA.foodRange;
  this.foodMult = this.DNA.foodMult;
...
Particle.prototype.flock = function(p,n,f) {
  ...
  sep.mult(this.seperateMult);
  ali.mult(this.alignMult);
  coh.mult(this.cohesionMult);
  
  run.mult(this.runMult);
  chase.mult(this.chaseMult);
  food.mult(this.foodMult);
...
Particle.prototype.lifeCycle = function(p){
  ...
  if(this.score > this.oldScore){
    ...
    if(this.team == 0){
      ...
      s = map(this.lifeSpan,0,this.lifeSpanMax,this.speed,this.speed-4);
    }
    else if(this.team == 1){
      b = map(this.lifeSpan,0,this.lifeSpanMax,255,0);
      s = map(this.lifeSpan,0,this.lifeSpanMax,this.speed,this.speed-0.5);
    }
  }
...
Particle.prototype.run = function(others) {
  var desiredseparation = this.runRange;
...
Particle.prototype.assignTeam = function(){
  ...
  if (this.team == 1){
    ...
    this.size = 12;
    this.maxSpeed = 5;
    this.lifeSpanMax = random(500,800);
    this.seperateSize = this.size * 3;
    this.chaseRange = 300;
